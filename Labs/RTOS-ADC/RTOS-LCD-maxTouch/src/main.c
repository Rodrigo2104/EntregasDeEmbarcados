...
SemaphoreHandle_t xSemaphore_afec;
...
static void AFEC_pot_Callback(void){
	...
	xSemaphoreGiveFromISR(xSemaphore_afec, &xHigherPriorityTaskWoken);
...
void task_lcd(void){
  ...
  while (true) {
	...
	if (xQueueReceive( xQueueADC, &(adc), ( TickType_t )  100 / portTICK_PERIOD_MS)) {
		...
		font_draw_text(&calibri_36, b, 100, 200, 1);
	}
  }
}
void task_adc(void){
	
	xSemaphore_afec = xSemaphoreCreateBinary();
	...
	while(1){
		if(xSemaphoreTake(xSemaphore_afec, (TickType_t) 100) == pdTRUE){
			printf("%d\n", g_ul_value);
