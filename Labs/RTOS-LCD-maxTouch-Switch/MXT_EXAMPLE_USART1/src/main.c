...
int process_touch(t_but botoes[], touchData touch, uint32_t n){
	...
	for (int i=0;i<n;i++){
		int l=botoes[i].x-botoes[i].width/2;
		int r=botoes[i].x+botoes[i].width/2;
		int u=botoes[i].y-botoes[i].height/2;
		int d=botoes[i].y+botoes[i].height/2;
		if (touch.x>=l && touch.x<=r && touch.y>=u && touch.y<=d){
			but=i;
			break;
		}
	}
